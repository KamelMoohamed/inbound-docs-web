...
export function PlanCard({ planKey, monthlyCredits, current, action, pending = false }:
  { planKey: string; monthlyCredits: number; current: boolean;
    action?: (formData: FormData) => void; pending?: boolean }) {
  ...
  return (
    <div className={`rounded-xl border p-5 ${current ? "border-indigo-500 ring-1 ring-indigo-200" : "border-slate-200"} bg-white`}>
      ...
      <form action={action} className="mt-4">
        ...
        <button type="submit" disabled={current || pending}
          ...
          {pending ? "Redirecting…" : current ? "Current plan" : `Choose ${planKey}`}
        </button>
      </form>
    </div>
  );
